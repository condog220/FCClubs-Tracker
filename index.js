...
async function getClubId(clubName) {
    try {
        const response = await fetch(`https://fc26-clubstracker.onrender.com/api/club?clubName=${encodeURIComponent(clubName)}`);
        if (!response.ok) {
...
async function getClubInfo(clubId) {
    try {
        const response = await fetch(`https://fc26-clubstracker.onrender.com/api/club/info?clubId=${encodeURIComponent(clubId)}`);
        if (!response.ok) {
...
async function getMemberStats(clubId) {
    try {
        const response = await fetch(`https://fc26-clubstracker.onrender.com/api/member?clubId=${encodeURIComponent(clubId)}`);
        if (!response.ok) {
